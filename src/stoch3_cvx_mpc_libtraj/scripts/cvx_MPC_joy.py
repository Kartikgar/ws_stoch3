...
import numpy as np
from controller_joy import mpc_controller
...
def joyCB(data):
    ...
    cmd_global['vx'] = -0.3 * data.axes[1]
    cmd_global['vy'] = -0.3 * data.axes[3]
    cmd_global['wz'] = -0.3 * data.axes[0]
...
def pidCB(data):
    ...
    pid_vel['vx'] = data.linear.x
    pid_vel['wz'] = data.angular.z
    pid_vel['vy'] = 0
def stateCB(data):
    global state_global
    state_global = data.data
...
if __name__ == '__main__':
    ...
    state_sub = rospy.Subscriber("/stoch3/state", String, stateCB, queue_size=1)
    leg_cmd_pub = rospy.Publisher('/stoch3/controller/leg_controller/command', QuadrupedLegCommand, queue_size=1)
    pid_sub = rospy.Subscriber('/cmd_vel', Twist, pidCB, queue_size =3 )
    i = 0
    ...
    while not rospy.is_shutdown():
        ...
        if state_global=='WalkingControl':
            state = Xt_global
